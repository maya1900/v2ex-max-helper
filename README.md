...
- 🔒 **隐私优先**：所有 Token、Chat ID、Cookie 均从环境变量或运行时文件读取，**不写入代码**；`TG_CHAT_ID` 可通过首次私聊自动绑定。
...
flowchart TD
    ...
    subgraph bot["Telegram Bot"]
        Q["Telegram 长轮询"] --> Q1{"已绑定授权用户?"}
        Q1 -->|"否"| Q2["首次私聊绑定<br/>可选 TG_SETUP_CODE"]
        Q1 -->|"是"| R{"命令?"}
        R -->|"余额"| S["/sou<br/>返回余额记录"]
        R -->|"Cookie"| T["/cookie<br/>导入或合并 Cookie"]
        R -->|"任务"| U["/checkin 或 /read<br/>手动触发任务"]
        R -->|"日志"| V["/debug<br/>配置日志级别"]
        R -->|"停止"| W["/stop<br/>SIGTERM 阅读进程"]
    end
```
