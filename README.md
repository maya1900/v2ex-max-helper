...
- 🔁 **保活心跳**：定时访问首页保持登录态，避免 Cookie 过早失效。
- 📖 **自动阅读**：真实浏览器（Playwright）阅读帖子，每篇随机停留 12~18 秒，刷活跃度铜币。
...
时间为 UTC，请按需换算（北京时间 = UTC + 8）。

```cron
# 每日 09:10（北京）签到
10 1 * * * /usr/bin/node /path/to/v2ex-max-helper/checkin/v2ex-checkin.js >> /var/log/v2ex.log 2>&1

# 每 6 小时保活一次，防 Session 过期
0 */6 * * * /usr/bin/node /path/to/v2ex-max-helper/checkin/v2ex-checkin.js --ping >> /var/log/v2ex.log 2>&1

# 每日 09:15（北京）开始自动阅读
15 1 * * * cd /path/to/v2ex-max-helper/reader && /usr/bin/node main.js >> /var/log/v2ex-reader.log 2>&1
```

更详细的部署说明见 [`docs/部署指南.md`](docs/部署指南.md)。
